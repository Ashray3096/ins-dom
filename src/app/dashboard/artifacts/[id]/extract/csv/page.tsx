 ...
 * CSV Extraction Page
 *
 * Visual selector for CSV files using table-based interface
 */

import { useEffect, useState } from 'react';
import { useParams, useRouter } from 'next/navigation';
import { CSVVisualSelector } from '@/components/template-builder/csv-visual-selector';
import type { CSVFieldMapping } from '@/components/template-builder/csv-visual-selector';
import { TemplateSaveModal } from '@/components/template-builder/template-save-modal';
import { Loader2 } from 'lucide-react';
import { toast } from 'sonner';
import type { Artifact } from '@/types/artifacts';

export default function CSVExtractionPage() {
  const params = useParams<{ id: string }>();
  const router = useRouter();
  const [artifact, setArtifact] = useState<Artifact | null>(null);
  const [loading, setLoading] = useState(true);
  const [pendingFieldMappings, setPendingFieldMappings] = useState<CSVFieldMapping[] | null>(null);
  ...
  const handleSave = async (fieldMappings: CSVFieldMapping[]) => {
    if (!artifact) return;
  ...
  const handleModalSave = async (templateName: string, templateDescription: string) => {
    ...
    try {
      ...
      toast.success(`Template "${templateName}" saved with ${fields.length} columns!`);
