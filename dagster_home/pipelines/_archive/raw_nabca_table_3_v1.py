"""
Auto-generated Dagster pipeline for raw_nabca_table_3
...
@asset(
    name="extract_raw_nabca_table_3",
    description="Extract YTD Sales data from NABCA PDFs using AWS Textract",
    compute_kind="extraction:textract",
    retry_policy=RetryPolicy(max_retries=3),
)
def extract_raw_nabca_table_3(context: AssetExecutionContext) -> Dict[str, Any]:
    """
    Extract raw_nabca_table_3 data from NABCA PDF artifacts.

    NABCA Section: YTD Sales
    Page Range: 7-8
    Extraction Method: AWS Textract (Table Detection)
    Entity Type: INTERIM
    """
    try:
        context.log.info(f"Starting NABCA extraction for raw_nabca_table_3")
        context.log.info(f"Section: YTD Sales, Pages: 7-8")
        ...
        for artifact in artifacts:
            try:
                ...
                # Extract specific page range (7-8)
                context.log.info(f"Extracting pages 7-8...")
...
@asset(
    name="load_raw_nabca_table_3",
    description="Load transformed data into raw_nabca_table_3 table",
    compute_kind="load",
    deps=["extract_raw_nabca_table_3"],
    retry_policy=RetryPolicy(max_retries=3),
)
def load_raw_nabca_table_3(
    context: AssetExecutionContext,
    extract_raw_nabca_table_3: Dict[str, Any]
) -> MaterializeResult:
    """
    Load raw_nabca_table_3 data into database.

    Target Table: raw_nabca_table_3
    Entity Type: INTERIM
    """
    try:
        context.log.info(f"Starting load for raw_nabca_table_3")
        ...
        records = extract_raw_nabca_table_3["records"]
        ...
        for i in range(0, len(clean_records), batch_size):
            ...
            try:
                response = supabase.table("raw_nabca_table_3").insert(batch).execute()
                ...
                for record in batch:
                    try:
                        supabase.table("raw_nabca_table_3").insert(record).execute()
                        loaded_count += 1
        ...
        return MaterializeResult(
            metadata={
                ...
                "table_name": MetadataValue.text("raw_nabca_table_3"),
            }
        )
