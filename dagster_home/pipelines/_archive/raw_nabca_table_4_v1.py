"""
Auto-generated Dagster pipeline for raw_nabca_table_4
...
@asset(
    name="extract_raw_nabca_table_4",
    description="Extract Rolling 12-Month data from NABCA PDFs using AWS Textract",
    compute_kind="extraction:textract",
    retry_policy=RetryPolicy(max_retries=3),
)
def extract_raw_nabca_table_4(context: AssetExecutionContext) -> Dict[str, Any]:
    """
    Extract raw_nabca_table_4 data from NABCA PDF artifacts.

    NABCA Section: Rolling 12-Month
    Page Range: 9-10
    Extraction Method: AWS Textract (Table Detection)
    Entity Type: INTERIM
    """
    try:
        context.log.info(f"Starting NABCA extraction for raw_nabca_table_4")
        context.log.info(f"Section: Rolling 12-Month, Pages: 9-10")
        ...
        for artifact in artifacts:
            try:
                ...
                # Extract specific page range (9-10)
                context.log.info(f"Extracting pages 9-10...")
        ...
        return {
            ...
            "metadata": {
                ...
                "nabca_section": "Rolling 12-Month",
                "page_range": "9-10",
            }
        }
...
@asset(
    name="load_raw_nabca_table_4",
    description="Load transformed data into raw_nabca_table_4 table",
    compute_kind="load",
    deps=["extract_raw_nabca_table_4"],
    retry_policy=RetryPolicy(max_retries=3),
)
def load_raw_nabca_table_4(
    context: AssetExecutionContext,
    extract_raw_nabca_table_4: Dict[str, Any]
) -> MaterializeResult:
    """
    Load raw_nabca_table_4 data into database.

    Target Table: raw_nabca_table_4
    Entity Type: INTERIM
    """
    try:
        context.log.info(f"Starting load for raw_nabca_table_4")
        ...
        records = extract_raw_nabca_table_4["records"]
        ...
        for i in range(0, len(clean_records), batch_size):
            ...
            try:
                response = supabase.table("raw_nabca_table_4").insert(batch).execute()
                ...
                for record in batch:
                    try:
                        supabase.table("raw_nabca_table_4").insert(record).execute()
                        loaded_count += 1
        ...
        return MaterializeResult(
            metadata={
                ...
                "table_name": MetadataValue.text("raw_nabca_table_4"),
            }
        )
