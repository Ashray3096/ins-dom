...
-- This stores auto-generated Dagster Python code for each entity's data pipeline
...
CREATE TABLE IF NOT EXISTS pipeline_deployments (
  ...
  python_code TEXT NOT NULL, -- Complete Python module code
  ...
  dagster_code_location TEXT, -- Dagster Cloud code location name
  dagster_deployment_id TEXT, -- ID from Dagster Cloud API
  deployment_status TEXT NOT NULL DEFAULT 'draft',
  -- Status: 'draft', 'deploying', 'active', 'failed', 'paused', 'archived'

  -- Deployment Tracking
  deployed_at TIMESTAMPTZ,
  deployed_by UUID REFERENCES auth.users(id),
  last_run_at TIMESTAMPTZ,
  last_run_status TEXT, -- 'SUCCESS', 'FAILURE', 'RUNNING', null
...
CREATE TABLE IF NOT EXISTS pipeline_runs (
  ...
  run_status TEXT NOT NULL, -- 'QUEUED', 'STARTING', 'RUNNING', 'SUCCESS', 'FAILURE', 'CANCELING', 'CANCELED'
...
CREATE INDEX IF NOT EXISTS idx_pipeline_deployments_pipeline_id ON pipeline_deployments(pipeline_id);
CREATE INDEX IF NOT EXISTS idx_pipeline_deployments_status ON pipeline_deployments(deployment_status);
CREATE INDEX IF NOT EXISTS idx_pipeline_deployments_created_by ON pipeline_deployments(created_by);
CREATE INDEX IF NOT EXISTS idx_pipeline_runs_deployment_id ON pipeline_runs(pipeline_deployment_id);
